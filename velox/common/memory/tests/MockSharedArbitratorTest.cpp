...
TEST_F(MockSharedArbitrationTest, noArbitratiognFromAbortedPool) {
  ...
  void* buf1 = reclaimedOp->allocate(128);
...
DEBUG_ONLY_TEST_F(MockSharedArbitrationTest, failedToReclaimFromRequestor) {
  ...
  for (const auto& testData : testSettings) {
    ...
    MockMemoryOperator* failedQueryOp = addMemoryOp(
        failedQuery, true, [&](MemoryPool* pool, uint64_t targetByt) {
    ...
    SCOPED_TESTVALUE_SET(
        ...
        std::function<void(const MemoryPool*)>(([&](const MemoryPool* pool) {
...
TEST_F(MockSharedArbitrationTest, memoryPoolAbortThrow) {
  ...
  MockMemoryOperator* largeQueryOp =
      addMemoryOp(largeQuery, true, [&](MemoryPool* pool, uint64_t targetByt) {
        VELOX_FAIL("throw reclaim exception");
      });
...
TEST_F(MockSharedArbitrationTest, raceBetweenInitialReservationAndArbitration) {
